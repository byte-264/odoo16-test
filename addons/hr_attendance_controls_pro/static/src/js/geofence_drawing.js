...
export class GeofenceDrawing extends Component {
    ...
    addLayerVector(){
        ...
        if (this.props.value && this.props.value != 'Empty') {                
            ...
            this.olmap.getView().fit(this.vectorSource.getExtent(), { duration: 100, maxZoom: 10 });
        }
    }
    ...
    updateMap() {
        ...
        this.addSearchNominatimControl();
    }
    ...
    addSearchNominatimControl(){
        ...
        self.olmap.addControl(geocoder);

        geocoder.on('addresschosen', (evt) => {
            geocoder.getSource().clear();
            const coords = [evt.place.lon, evt.place.lat];
            const accuracy = ol.geom.Polygon.circular(coords, 0);
            self.vectorSource.clear(true);
            self.vectorSource.addFeatures([
