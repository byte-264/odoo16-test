odoo.define('dynamic_odoo.basic_widgets', function (require) {
    ...
    var {mount, Component, xml} = owl;
    const {templates} = require("@web/core/assets");
    var legacyEnv = require('web.commonEnv');
    // var {useBus, useService, useOwnedDialogs} = require("@web/core/utils/hooks");
    // var {DomainSelectorDialog} = require("@web/core/domain_selector_dialog/domain_selector_dialog");
    var QWeb = core.qweb;
    ...
    var _ModelFieldSelector = ModelFieldSelector.extend({
        init: function (parent, model, chain, options) {
            ...
            this.props = options;
            const self = this, spSelectField = this.popover._selectField.bind(this.popover);
            this.popover._selectField = (field) => {
                spSelectField(field);
                self._selectField(field);
            }
        },
    ...
    var ChooseField = base.WidgetBase.extend({
        ...
        _renderView: async function () {
            ...
            const viewContainer = await mount(ComponentAdapter, this.$el.find(".wCon").empty()[0], {
                env: legacyEnv,
                templates: templates,
                props: {
                    Component: _ModelFieldSelector,
                    widgetArgs: [model || false, chain, fsOptions]
                },
                position: "first-child"
            });
            self.bindStyle();
            this.ref.fieldSelector = viewContainer.widget;
        },
    });

    var ChooseFieldDialog = base.WidgetBase.extend({
        renderView: function () {
            const {onChange, onClose} = this.props, $wrap = $("<div>");
            ...
            this.dialog.on_attach_callback = function () {
                chooseField.appendTo($wrap);
            }
            this.dialog._opened = this.dialog.opened(this.dialog.on_attach_callback.bind(this.dialog));
            this.dialog.open();
    ...
    var ButtonDomain = base.WidgetBase.extend({
        ...
        //         resModel: model,
        //         initialValue: this.getValDomain(),
        //         readonly: false,
        //         isDebugMode: true,
        //         onSelected: (domain, isDebugEdited) => {},
        //     });
        // },
        renderDomainSelector: async function () {
            const self = this, {model, onClose, options} = this.props;
            //
            ...
            const viewContainer = await mount(ComponentAdapter, $(".o_action_manager")[0], {
                env: legacyEnv,
                templates: templates,
                props: {
                    Component: DomainSelectorDialog,
                    widgetArgs: [model, this.getValDomain(), {readonly: false, debugMode: true, ...options}]
                },
                position: "first-child"
            });
            viewContainer.widget.on('closed', this, function () {
                self.change ? (onClose || function () {
                })() : self.setValue(self.state.value);
                self.change = false;
            });
            const _onDomainChange = viewContainer.widget._onDomainChange.bind(viewContainer.widget);
            viewContainer.widget._onDomainChange = function (e) {
                _onDomainChange(e);
                self.setValue(Domain.prototype.arrayToString(viewContainer.widget.newDomain));
                self.change = true;
            }
            viewContainer.widget.open();
            this.ref.domainSelector = viewContainer.widget;
        },
    });
    ...
    var OptionsEdit = base.WidgetBase.extend({
        ...
        init: function (parent, params) {
            ...
            this.state = {value: null, default_value: false, options: options || []};
            this.newOptions = [];
        },
        ...
        onSetDefault: function (e) {
            e.stopPropagation();
            const el = $(e.currentTarget), optionKey = el.parents(".item").attr("data_id");
            this.setState({default_value: optionKey});
            this.renderOptions();
            this.bindAction();
        },
        ...
        bindAction: function () {
            ...
            if (this.$content) {
                ...
                this.$content.find(".cbCheck").click(this.onSetDefault.bind(this));
        ...
        renderOptions: function () {
            const {options, default_value, active} = this.state, wTask = this.$content.find(".wContent").empty();
        ...
        onSave: function () {
            const {onChange} = this.props, {options, default_value} = this.state;
            if (onChange) {
    ...
    var ChangeImage = base.WidgetBase.extend({
        ...
        onChangeImage: function (e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            const {onChange} = this.props, MediaDialog = odoo.__DEBUG__.services['wysiwyg.widgets.Dialog'],
                $image = $('<img/>');
        ...
        bindAction: function () {
            this._super();
            this.$el.find("a").click(this.ok.bind(this));
        }
    })
