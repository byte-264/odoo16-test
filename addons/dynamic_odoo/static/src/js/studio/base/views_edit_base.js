odoo.define('dynamic_odoo.views_edit_base', function (require) {
    ...
    const {deepCopy} = require("@web/core/utils/objects");
    ...
    var NewNodeStore = base.WidgetBase.extend({
        ...
        stringXmlToJSON: function (el, clean = false) {
            return eval('utils.xml_to_json((new DOMParser()).parseFromString(el, "text/xml"), clean)');
        },
        ...
        newGrid: function (props = {}) {
            return this.stringXmlToJSON(QWeb.render("Studio.Form.Component.Grid", props.props), true);
        },
    ...
    var ViewContent = EditContent.extend({
        ...
        _prepareDataSave: function () {
            const {viewInfo, parentView} = this.props, {
                    viewId,
                    view_id,
                    view_key,
                    parentField,
                ...
                viewKey = view_key || this.getViewKey(), {action} = $.bbq.getState(true), res = {
                    view_type: this.getViewType(type),
                    action_id: action,
            ...
            if (!res.view_id) {
                if (parentField.fieldNode.attrs.original_context == undefined) {
                    parentField.fieldNode.attrs.original_context = deepCopy(parentField.fieldNode.attrs.context || "{}");
                }
                let _context = (parentField.fieldNode.attrs.original_context || "{}").trim();
                parentField.fieldNode.attrs.context = String(`{'${res.view_type}_view_ref': '${'odoo_studio.' + res.view_key}',`) + _context.substring(1);
        ...
        prepareDataToSave: function () {
            ...
            viewInParent.map((view) => {
                ...
                const _data = view._prepareDataSave();
                res.push(_data);
                if (arch_base) {
                    ...
                    Object.values(fieldInfo.views).map((view) => {
                        res.push({
                            arch: view.arch_base,
                            force: true,
                            parent_stack_id: view.stack_id,
                            action_id: action,
                            view_type: this.getViewType(view.type),
                            view_key: this.getViewKey(),
                            res_model: viewInfo.model,
                            parent_view_key: _data.view_key
                        });
                    });
                    fieldNode.attrs.view_key = _data.view_key;
        ...
        prepareViewInParent: async function () {
            ...
            if (onGet) {
                const view_base = await onGet([['action_id', '=', action], ['parent_view_key', '=', fieldNode.attrs.view_key], ['view_type', 'in', [this.getViewType(type)]]]);
                view_base.map((view) => {
                    if (view.arch) {
        ...
        setNodeId: function (node, loop = false) {
            if (!node.tag) {
                return;
            }
        ...
        //     const MediaDialog = odoo.__DEBUG__.services['wysiwyg.widgets.Dialog'], $image = $('<img/>');
        ...
        renderViewContent: async function () {
            ...
            const {viewInfo} = this.props, {context} = this.action, {model, relatedModels, fields, type} = viewInfo;
            const props = {
                resModel: model,
                type: type,
                context: Object.assign(context || {}, {Studio: true}),
            ...
            const viewContainer = await mount(ViewContainer, this.$el.find(".viewView").empty()[0], {
            ...
            // if (!this.isLegacy) {
            //     // Object.values(viewContainer.__owl__.children)[0].childEnv.legacyRefs.widget
            //     this.ref.widget = component;
            // }
            this.ref.widget = this.isLegacy ? Object.values(viewContainer.__owl__.children)[0].childEnv.legacyRefs.widget : component;
        },
        ...
        destroy: function () {
            if (this.ref.container.__owl__) {
                this.ref.container.__owl__.destroy();
            }
        }
    });
