odoo.define('dynamic_odoo.form', function (require) {
    ...
    var FormEditRenderer = FormRenderer.extend({
        ...
        _renderInnerGroup: function (node) {
            ...
            let res = this._super(node), children = node.children, wrong_list = [];
            children.map((child) => {
                if (child.tag) {
                    child.parentId = node.nodeId;
                }
        ...
        _updateView: function ($newContent) {
            const formNodeId = $newContent.parents(".o_legacy_form_view").attr("node-id");
            if (formNodeId) {
            ...
            if (this.loadParams && this.loadParams.fieldsInfo) {
                ...
                _.each(this.loadParams.fieldsInfo.form, function (attrs, fieldName) {
                    ...
                    if (attrs.Widget.prototype.useSubview && !attrs.__no_fetch && !attrs.views[attrs.mode]) {
                        ...
                        defs.push(odoo.studio.instance.loadViewsSubView(
                            field.relation,
    ...
    var FormProps = baseEdit.ViewProps.extend({
        init: function (parent, props) {
            ...
            this.nodeProps.default_value = {
                name: 'default_value',
                valType: 'string',
                label: 'Default',
                prepareProps: this.prepareDefaultValue.bind(this),
                propChange: this.onChangeDefaultValue.bind(this),
            ...
            this.nodeProps.show_chatter = {
                name: "show_chatter",
                prepareProps: this.prepareChatterWidget.bind(this),
                valType: "boolean",
                label: "Show Chatter Widget",
                propChange: this.onShowChatterWidget.bind(this),
                widget: basic_fields.ToggleSwitch
            };
            ...
            this.viewPropsView = ["create", "edit", "delete", "show_invisible", "show_chatter"];
        ...
        prepareFieldView: function (node) {
            ...
            const fieldViewStore = {
                    ...
                    selection: ["required", "invisible", "readonly", "string", "selection", "default_value", "widget", "groups", "help", "context"],
        ...
        prepareDefaultValue: function (node, nodeProps) {
            const selection = eval(node.attrs.selection || this.props.viewInfo.fields[node.attrs.name].selection || []);
            nodeProps.options = selection.map((op) => ({label: op[1], value: op[0]}));
        },
        onChangeDefaultValue: function (node, prop, value) {
            node.attrs.default_value = value;
            this.default_values[node.attrs.name] = '"' + value + '"';
        },
        ...
        prepareChatterWidget: function (node, nodeProps) {
            if (node.children.length) {
                let isChatter = node.children[node.children.length - 1];
                nodeProps.value = (isChatter.attrs.class || "").includes("oe_chatter") ? true : false;
            }
        },
        onShowChatterWidget: function (node, prop, value) {
            value = this.preparePropVal(prop, value);
            let lastChild = node.children[node.children.length - 1],
                isChatter = (lastChild.attrs.class || "").includes("oe_chatter") ? true : false;
            if (value && !isChatter) {
                const {newNode} = this.nodeStore;
                const chatterNode = newNode.bind(this.nodeStore)({
                    tag: "div",
                    children: [
                        newNode.bind(this.nodeStore)({
                            tag: "field",
                            props: {name: "message_follower_ids", groups: "base.group_user"}
                        }),
                        newNode.bind(this.nodeStore)({
                            tag: "field",
                            props: {name: "activity_ids"}
                        }),
                        newNode.bind(this.nodeStore)({
                            tag: "field",
                            props: {name: "message_ids"}
                        }),
                    ],
                    props: {class: "oe_chatter"}
                });
                node.children.push(chatterNode);
            } else if (!value && isChatter) {
    ...
    var FormEditContent = baseEdit.ViewContent.extend({
        ...
        init: function (parent, params) {
            ...
            this.sortParams.selector = [
                ["._wComField ._wSortable, .wFields, .o_inner_group > tbody, ._wPage", ".o_inner_group > tbody, ._wPage"],
        ...
        _prepareDataSave: function () {
            const newFields = this.prepareNewFields(), res = this._super(), {viewProps} = this.ref;
            res.button_data = this.buttonData;
            res.new_fields = newFields;
            res.default_values = viewProps.default_values;
            viewProps.default_values = {};
            return res;
        },
        prepareNewFields: function () {
            ...
            return Object.values(this.newFields).map((field) => {
                ...
                if (default_value) {
                    valField.default_value = default_value;
                }
        ...
        onShowSubView: async function (e) {
            ...
            if (renderSubView) {
                ...
                if (!subViewInfo) {
                    ...
                    let _context = {action: $.bbq.getState(true).action}, regex = /'([a-z]*_view_ref)' *: *'(.*?)'/g,
                        matches;
        ...
        onChangeProp: function (node, prop = {}) {
            const self = this;
            if (prop.name == "show_invisible") {
                        ...
                        nodeProps.props = Object.assign(nodeProps.props,
                            {
                                ...
                                _new: true,
                                type: nodeProps.type, name: "x_" + this.getRandom()
                            });
