    ...
    var ViewsCenter = BaseCenter.extend({
        ...
        setFieldsInfo: function (viewInfo, fieldWithout = []) {
            const self = this, {fields, type} = viewInfo, fieldsInfo = viewInfo.fieldsInfo[type],
                models = this.views_info.relatedModels,
                ...
                if (!fieldsNode.includes(fieldName) && !fieldWithout.includes(fieldName)) {
                    ...
                    if (["one2many", "many2many"].includes(field.type)) {
                        Object.values(fieldInfo.views).map((view) => {
                            const fieldsGet = models[field.relation], {viewFields, fields} = view,
                                fieldsName = Object.keys(fields);
                            view.inParent = true;
        ...
        getViewInfo: function () {
            const self = this, {viewType} = this.state, {fields} = this.views_info;
            if (viewType in self.fieldsViews) {
                ...
                return {
                    ...
                    fields: {...fields},
                    fieldsGet: {...fields},
                    relatedModels: {...this.views_info.relatedModels}
                };
            }
        },
        viewTemplate: function (viewName) {
            const view = {kanban: "<kanban><field name='id' /><templates><t t-name='kanban-box'><div t-attf-class='oe_kanban_card oe_kanban_global_click'></div></t></templates></kanban>"};
        ...
        prepareNewView: function () {
            ...
            data.arch = viewCheck && !viewCheck.viewId ? viewCheck.arch : (this.viewTemplate(viewType) || QWeb.templates[viewTemplate].children[0].outerHTML);
        ...
        onClickCard: async function (e) {
            ...
            if (this.ref.view) {
                const {viewType} = this.ref.view.props.viewInfo;
                if (viewType == "pivot") {
                    this.ref.view.destroy();
                }
            }
            this.renderElement();
        },
        ...
        onClose: async function (e) {
            ...
            if (this.action.res_model == "res.config.settings" || !this.validAction()) {
        ...
        loadViews: async function (callback = () => {
            ...
            const params = {
                ...
                context: {...this.action.context, action: this.action.id, studioKey: Math.random()},
                ...viewParams
            };
            const options = {
            ...
            this.views_info = await this.vmSt.loadViews(params, options);
            this.fieldsViews = this.views_info.__legacy__.fields_views;
            callback();
        },
        bindAction: function () {
            this._super();
        ...
        loadViewsSubView: async function (resModel, context, views) {
            return (await this.vmSt.loadViews({resModel, views, context}, {})).__legacy__.fields_views;
        },
        ...
        renderSubView: async function (viewInfo) {
            ...
            widgetView.on_attach_callback();
            widgetView.inDOM = true;
        },
        renderViewViews: async function (wrap) {
            ...
            if (viewType in this.fieldsViews) {
                ...
                this.ref.viewRoot.on_attach_callback();
                this.ref.viewRoot.inDOM = true;
            } else {
                this.activeConfirm();
            }
        },
