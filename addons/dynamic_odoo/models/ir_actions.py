...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_html(self, report_ref, docids, data=None):
        report_name = report_ref['report_name'] if hasattr(report_ref, "name") else report_ref
        return super(IrActionsReport,
                     self.with_context(REPORT_ID=self.search([('report_name', '=', report_name)], limit=1).id))._render_qweb_html(
    ...
    def copy_report(self):
        ...
        studio_view = self.env['report.center'].search(
            [('view_id', '=', template_view.id), ('report_id', '=', self.id)], limit=1)
        if len(studio_view):
            studio_view.copy({'view_id': new_template.id, 'report_id': new.id})
        new.write({
