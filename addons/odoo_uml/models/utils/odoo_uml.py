...
class UtilMixin(object):
    ...
    def _produce_diagram_image(self):
        ...
        f_out_puml
        f_out_puml.close()
        nombre=f_out_puml.name
        cmdline=['java','-XX:MaxHeapSize=500m','-XX:CompressedClassSpaceSize=168m','-DPLANTUML_LIMIT_SIZE=9192', '-jar', PLANT_UML_PATH, '-tpng', '-v', nombre,'-o', '"%s"' % path.dirname(f_out_puml.name)]
        #if not path.exists(PLANT_UML_PATH):
        #    raise Exception('jar file not found!')
        #popen = Popen(['java', '-jar', PLANT_UML_PATH, '-tpng', '-v',   f_out_puml.name],   stdout=PIPE)

        image=False
        log =False
        call(cmdline)
        try:
            f_out_diagram = open(file=f_out_puml.name+".png" , mode='rb')
...
class ClassDiagram(PlantUMLClassDiagram, UtilMixin):
    ...
    def record_field(field):
        ...
        '''
        record=ClassDiagram.record_model(field.model_id)._fields
    ...
    def produce_model_features_options(self, model, **kwargs):
        ...
        if kwargs.get('show_model_config_options', True):
            if rec._inherit is not None:
                if isinstance(rec._inherit, basestring):
            ...
            if rec._inherits:
                ...
                print(rec._inherits)
                for model, field in rec._inherits.items():
        ...
        for field in model.field_id.filtered(lambda r: r.ttype in ['many2one', 'one2many']):
            ...
            if field.ttype == 'one2many' and module_2o.name != self._module.name:
                ...
                relation = dict(
                    alias1=alias2,
                    alias2=alias1,
                    card1='{0} *'.format(inverse) if inverse is not None else '*',
                    card2='{0} 1'.format(field.name),
                    name='<<restrict>>' if field.on_delete == 'restrict' else None,
                    inverse=inverse
                )
    ...
    def produce_inherits_relations(self, model, **kwargs):
        rec = ClassDiagram.record_model(model)
        for inherit, field in rec._inherits.items():
    ...
    def _resolve_module(self, model, near=True):
        ...
        '''
        model_name = model if isinstance(model, basestring) else model.model
