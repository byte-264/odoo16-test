odoo.define('dashboard_studio.DashboardsCenter', function (require) {
    ...
    var DashboardsCenter = BaseCenter.extend({
        init: function (parent, params) {
            ...
            this.categorys.all = {
                name: "all", label: "All Widgets", views: ["battery", "title", "todo", "graph",
                    "pivot", "list", "text", "countdown", "calendar", "youtube", "bookmark"]
            };
            this.categorys.recommended = {
                name: "recommended",
                label: "Recommended",
                views: ["battery", "title", "todo", "graph", "calendar", "pivot", "text"]
            };
            this.categorys.staying_on_top = {
                name: "staying_on_top",
                label: "Staying on Top",
                views: ["battery", "title", "list", "graph", "calendar", "pivot"]
            };
            ...
            this.viewWidgets.calendar = Calendar;
        ...
        prepareNewView: function () {
            ...
            const viewTemplate = `DashboardStudio.Widget.${viewType}`,
                arch = QWeb.templates[viewTemplate].children[0].outerHTML,
        ...
        reloadView: function () {
            ...
            this.controlModel.loadDashboard(record.id, record.view_id[0]).then((viewInfo) => {
                let _viewInfo = self.controlView._processFieldsView(viewInfo[0].viewInfo, viewType)
                if (_viewInfo.type == "list") {
                    const {fields} = _viewInfo, fieldsInfo = _viewInfo.fieldsInfo["list"],
                        fieldWithout = {list: ["activity_exception_decoration"]};
                    const withoutField = fieldWithout["list"] || [];
                    Object.keys(fields).map((fieldName) => {
                        if (!(fieldName in fieldsInfo) && !withoutField.includes(fieldName)) {
                            ...
                            self.controlView._processNode(fieldNode, _viewInfo);
                            _viewInfo.fieldsInfo["list"][fieldName].willShow = true;
                        }
                    });
                }
        ...
        _getViewInfo: function (viewInfo, force = false) {
            var self = this, {arch, type} = viewInfo;
            if (force || (typeof viewInfo.arch == "string")) {
        ...
        renderViewSetup: function () {
            ...
            if (viewType && record.id) {
                ...
                widgetView.noRender = true;
                widgetView.appendTo(this.$el.find(".wgCon").empty()).then(() => {
                    widgetView.renderViewContent();
                    widgetView.noRender = false;
                });
