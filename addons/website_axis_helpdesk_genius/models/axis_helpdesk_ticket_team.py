...
class axisHelpdeskTeam(models.Model):
    ...
    def _default_domain_res_user_ids(self):
        return [('company_id','in',[self.env.company.id or False]),('groups_id', 'in', self.env.ref('website_axis_helpdesk_genius.group_helpdesk_ticket_users').sudo().id)]
    ...
    res_user_ids = fields.Many2many("res.users",'res_user_help_team','user_id','team_id',string="Team Members",domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    ...
    assigning_method = fields.Selection([
        ...
        default='manual', required=True,compute='_compute_assign_method', store=True, readonly=False,compute_sudo=True)
    visibility_res_user_ids = fields.Many2many('res.users', 'helpdesk_visibility_team', string='Team Visibility',
                                             domain=lambda self: self._default_domain_res_user_ids())
    ...
    upcoming_ticket_sla_policy_fail_tickets = fields.Integer(string='Upcoming SLA Fail Tickets', compute='_compute_fail_sla_tickets',compute_sudo=True)
    use_rating = fields.Boolean('Ratings on tickets')
    portal_rating_url = fields.Char('URL to Submit an Issue', readonly=True, compute='_compute_portal_rating_url',compute_sudo=True)
    ...
    def _onchange_res_user_ids(self):
        team_id = self.env['axis.helpdesk.ticket.team'].sudo().search([])
        user_ids = self.env['res.users'].sudo().search([('id','!=',2)])
        lst = []
        for team in team_id:
            for user in user_ids:
                if user.has_group('website_axis_helpdesk_genius.group_helpdesk_ticket_users') or user.has_group(
                        'website_axis_helpdesk_genius.group_helpdesk_ticket_manager'):
                    user_manager = self.env['res.users'].sudo().search([('id', '=', user.id)])
                    for manager in user_manager:
                        for member in team.res_user_ids.ids:
                            if manager.id == member:
                                lst.append(team.id)
                        manager.sudo().write({'helpdesk_team_ids': [(6, 0, lst)]})
    ...
    def _compute_assign_method(self):
        ...
        with_manual.sudo().update({'assigning_method': 'manual'})
    ...
    def _ticket_stage_define(self):
        ...
        for team in self:
            result[team.id] = self.env['axis.helpdesk.stage'].sudo().search([('team_ids', 'in', team.id)], order='sequence',
                                                                limit=1)
        return result
        ...
        for team in self:
            ...
            if res_user_ids:
                if team.assigning_method == 'randomly':  # randomly means new tickets get uniformly distributed
                    last_assigned_user = self.env['axis.helpdesk.ticket'].sudo().search([('helpdesk_team_id', '=', team.id)],
                    ...
                    result[team.id] = self.env['res.users'].sudo().browse(res_user_ids[index])
                elif team.assigning_method == 'balanced':  # find the member with the least open ticket
                    ticket_count_data = self.env['axis.helpdesk.ticket'].sudo().read_group(
                    ...
                    result[team.id] = self.env['res.users'].sudo().browse(
    ...
    def _compute_fail_sla_tickets(self):
        ticket_id = self.env['axis.helpdesk.ticket'].sudo().read_group([
    ...
    def stage_set(self):
        result = dict.fromkeys(self.ids, self.env['axis.helpdesk.stage']).sudo()
        for team in self:
            result[team.id] = self.env['axis.helpdesk.stage'].sudo().search([('team_ids', 'in', team.id)],
    ...
    def assign_user_to_team(self):
        ...
        for team in self:
            ...
            if res_user_ids:
                if team.assigning_method == 'randomly':  # randomly means new tickets get uniformly distributed
                    last_assigned_user = self.env['axis.helpdesk.ticket'].sudo().search([('team_id', '=', team.id)], order='create_date desc, id desc', limit=1).res_user_id
                    index = 0
                    if last_assigned_user and last_assigned_user.id in res_user_ids:
                        previous_index = res_user_ids.index(last_assigned_user.id)
                        index = (previous_index + 1) % len(res_user_ids)
                    ...
                    ticket_count_data = self.env['axis.helpdesk.ticket'].sudo().read_group([('helpdesk_stage_id.is_close', '=', False), ('res_user_id', 'in', res_user_ids)], ['res_user_id'], ['res_user_id'])
                    open_ticket_per_user_map = dict.fromkeys(res_user_ids, 0)  # dict: user_id -> open ticket count
                    open_ticket_per_user_map.update((item['res_user_id'][0], item['user_id_count']) for item in ticket_count_data)
                    result[team.id] = self.env['res.users'].sudo().browse(min(open_ticket_per_user_map, key=open_ticket_per_user_map.get))
        return result
    ...
    def filter_stage_data_dashboard(self):
        ...
        sla_complete = self.env['axis.helpdesk.ticket'].sudo().search_read(domain=[('helpdesk_stage_id', '=', 'Done'),('helpdesk_sla_deadline','<=',datetime_object),('company_id','in',context.get('allowed_company_ids')) or ('company_id','=',False)])
        sla_pending = self.env['axis.helpdesk.ticket'].sudo().search_read(domain=[('helpdesk_stage_id', '!=', 'Done'),('helpdesk_sla_deadline','>=',datetime_object),('company_id','in',context.get('allowed_company_ids')) or ('company_id','=',False)])
        sla_missed = self.env['axis.helpdesk.ticket'].sudo().search_read(domain=[('helpdesk_stage_id', '=', 'Done'),('helpdesk_sla_deadline','<=',datetime_object),('company_id','in',context.get('allowed_company_ids')) or ('company_id','=',False)])
        ...
        team_id = self.env['axis.helpdesk.ticket.team'].search_read(['|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)])
        ...
        if self.env['ir.config_parameter'].sudo().get_param('filter_ids'):
            ...
            filter_ids = self.env['axis.helpdesk.stage'].browse(filter_ids)
        else:
            filter_ids = []
        stage_dict = {}
        filter_dict = {}
        ...
        if user_uses_sla:
            ...
            group_fields.insert(3, 'helpdesk_sla_late')
            list_fields.insert(1, 'helpdesk_sla_deadline')
            list_fields.insert(2, 'helpdesk_sla_late')
        ...
        tickets = HelpdeskTicket.search_read(expression.AND([domain, [('helpdesk_stage_id.closed', '=', False),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)]]),
                                             ['helpdesk_sla_deadline', 'closed_hours', 'helpdesk_sla_late', 'priority'])
        all_tickets = HelpdeskTicket.search_read(expression.AND([domain, [('name', '!=', False),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)]]),
                                                 ['helpdesk_sla_deadline', 'closed_hours', 'helpdesk_sla_late', 'priority'])
        closed_tickets = HelpdeskTicket.search_read(expression.AND([domain, [('helpdesk_stage_id.closed', '=', True),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)]]),
                                                    ['helpdesk_sla_deadline', 'closed_hours', 'helpdesk_sla_late', 'priority'])
        user_company_id = self.env.user.company_id
        stage_ids = self.env['axis.helpdesk.stage'].sudo().search_read([('id', 'in', user_company_id.helpdesk_stage_ids.ids),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)])
        cr = self._cr
        ...
        for ticket_data in ticket_id_all_data:
            stage_id = self.env['axis.helpdesk.stage'].sudo().search([('id', '=', ticket_data['helpdesk_stage_id']),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)])
            partner_id = self.env['res.partner'].sudo().search([('id', '=', ticket_data['partner_id']),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)])
            user_id = self.env['res.users'].sudo().search([('id', '=', ticket_data['res_user_id']),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)])
            ticket_dic = {
        ...
        for stage_id in stage_ids:
            ticket_id = self.env['axis.helpdesk.ticket'].sudo().search([('helpdesk_stage_id', '=', stage_id['id']),'|',('company_id','in',context.get('allowed_company_ids')) , ('company_id','=',False)])
            data_dic = {
        ...
        def failed_sla_ticket(data):
            ...
            return helpdesk_sla_deadline or data.get('helpdesk_sla_late')
