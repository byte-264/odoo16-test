...
def search_helpdesk_tickets(self, **kwargs):
    ...
    return request.render('website_axis_helpdesk_genius.helpdesk_ticket_search', {'ticket': ticket_ids})
...
class ViewPortalHelpdesk(CustomerPortal):
    ...
    def axis_helpdesk_ticket(self, page=1, date_start=None, date_end=None, sortby=None, filterby='all', search=None,
                             groupby='none', search_in='content', **kw):
        values = self._prepare_portal_layout_values()
        axis_ticket_search_sort_data = {
        ...
        axis_ticket_search_filter_data = {
        ...
        if not sortby:
            sortby = 'date'
        order = axis_ticket_search_sort_data[sortby]['order']
        ...
        if filterby in ['last_message_sup', 'last_message_cust']:
            ...
            domain = axis_ticket_search_filter_data[filterby]['domain']
        ...
        if search and search_in:
            ...
            if search_in in ('content', 'all'):
                axis_ticket_ids = OR(
                    [axis_ticket_ids, ['|', ('name', 'ilike', search), ('description', 'ilike', search)]])
            if search_in in ('customer', 'all'):
                axis_ticket_ids = OR([axis_ticket_ids, [('partner_id', 'ilike', search)]])
            if search_in in ('message', 'all'):
                discussion_subtype_id = request.env.ref('mail.mt_comment').id
                axis_ticket_ids = OR([axis_ticket_ids, [('message_ids.body', 'ilike', search),
                                                        ('message_ids.subtype_id', '=', discussion_subtype_id)]])
            if search_in in ('status', 'all'):
                axis_ticket_ids = OR([axis_ticket_ids, [('stage_id', 'ilike', search)]])
        ...
        pager = portal_pager(
            url="/my/tickets",
            url_args={'date_start': date_start, 'date_end': date_end, 'sortby': sortby, 'search_in': search_in,
                      'search': search},
        ...
        tickets = request.env['axis.helpdesk.ticket'].search(domain, order=order, limit=self._items_per_page,
                                                             offset=pager['offset'])
        request.session['my_tickets_history'] = tickets.ids[:100]

        if groupby == 'stage':
            ticket_agam_help_id = [request.env['axis.helpdesk.ticket'].concat(*g) for k, g in
        ...
        values.update({
            ...
            'pager': pager,
            'axis_ticket_search_sort_data': axis_ticket_search_sort_data,
            'axis_ticket_search_filter_data': axis_ticket_search_filter_data,
            'axis_ticket_search_status_data': axis_ticket_search_status_data,
            'axis_ticket_search_group_data': axis_ticket_search_group_data,
            'sortby': sortby,
            'groupby': groupby,
            'search_in': search_in,
            'search': search,
        ...
        return request.render("website_axis_helpdesk_genius.view_helpdesk_ticket_portal", values)
    ...
    def helpdesk_ticket_view_form_portal(self, ticket_id=None, access_token=None, signature=None, **kw):
        print("signature_+++++++++++++++++++", signature)
        try:
        ...
        return request.render("website_axis_helpdesk_genius.axis_helpdesk_ticket_portal_view", values)
    ...
    def index_submit(self, access_token=None, **post):
        ...
        return request.render('website_axis_helpdesk_genius.rating_submit')
...
class axisHelpdeskTicket(http.Controller):
    ...
    def search_create_helpdesk_tickets_details(self, **kwargs):
        ...
        return request.render('website_axis_helpdesk_genius.helpdesk_ticket_search', {'ticket': helpdesk_tickets})
    ...
    def helpdesk_search_ticket(self, **kwargs):
        ...
        if ticket_id:
            ...
            return request.render('website_axis_helpdesk_genius.helpdesk_error_message', {'error_message': ticket_id})
    ...
    def helpdesk_form(self, **post):
        ...
        if user:
            ...
            partner_name = ""
            partner_email = ""
        return request.render("website_axis_helpdesk_genius.helpdesk_create_ticket_from_front_end",
    ...
    def helpdesk_form_submit(self, **post):
        tickettype_id = request.env['axis.helpdesk.ticket.type'].sudo().search(
            [('id', '=', post.get('ticket_type_id'))], limit=1)
        res_user_id = ''
        ...
        return request.render("website_axis_helpdesk_genius.view_helpdesk_ticket_sla_policy_success", vals)
...
class TicketRating(http.Controller):
    ...
    @http.route(['/helpdesk/rating', '/helpdesk/rating/<model("axis.helpdesk.ticket.team"):team>'], type='http',
                auth="public", website=True, sitemap=True)
    def page(self, team=False, **kw):
        user = request.env.user
        team_domain = [('id', '=', team.id)] if team else []
        if user.has_group('website_axis_helpdesk_genius.group_helpdesk_ticket_manager'):
        ...
        return request.render('website_axis_helpdesk_genius.view_helpdesk_ticket_rating',
                              {'page_name': 'rating', 'teams': team_values})



class CustomerPortal(portal.CustomerPortal):

    @http.route(['/axis/helpdesk/ticket/<int:ticket_id>/accept'], type='json', auth="public", website=True, csrf=False)
    def portal_ticket_accept(self, ticket_id, access_token=None, name=None, signature=None):
        # get from query string if not on json param
        access_token = access_token or request.httprequest.args.get('access_token')
        print('signature+__________________',signature)
        print('name+__________________',name)
        print('access_token+__________________',access_token)


        ticket = request.env['axis.helpdesk.ticket'].sudo().browse(ticket_id)
        if signature:
            ticket.write({'signature': signature})

        query_string= ''
        return {
            'force_refresh': True,
            'redirect_url': ticket.get_portal_url(query_string=query_string),
        }


    # @http.route(['/axis/helpdesk/ticket/<int:ticket_id>/accept'], type='json', auth="public", website=True)
    # def portal_quote_accept(self,  access_token=None, signature=None):
    #     # get from query string if not on json param
    #     access_token = access_token or request.httprequest.args.get('access_token')
    #     print('signature+__________________', signature)
    #     errrrr
